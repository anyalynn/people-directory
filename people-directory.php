...
function people_admin_init(){
		...
		add_meta_box('degrees', 'Degrees', 'degree_callback', 'people', 'side', 'low');
	...
	function degree_callback() {
		global $post;
		$custom = get_post_custom($post->ID);
		$position = $custom['degree'][0];
		?><input name="degree" value="<?= $degree ?>" /><?php
	}
	...
	function save_person_details() {
		...
		if (get_post_type($post) == 'people') {
			...
			update_post_meta($post->ID, 'degree', $_POST['degree']);
